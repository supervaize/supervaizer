...
- **Supervaizer v2 runtime hardening** — Added typed common fields to v2 action effects, including resource import counts, rows, errors, gaps, summaries, and case snapshots; validated `job_state` on action results; authenticated the A2A SSE event stream; sanitized A2A handler errors; and constrained legacy method execution to declared non-blocked method paths.
