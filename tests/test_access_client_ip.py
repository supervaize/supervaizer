...
class TestExtractClientIpNoTrustedProxies:
    ...
    def test_returns_peer_ip(self: "TestExtractClientIpNoTrustedProxies") -> None:
        scope = _make_scope("1.2.3.4")
        assert _extract_client_ip(scope) == "1.2.3.4"

    def test_ignores_xff_when_no_trusted_proxies(self: "TestExtractClientIpNoTrustedProxies") -> None:
        scope = _make_scope("1.2.3.4", xff="9.9.9.9, 8.8.8.8")
        assert _extract_client_ip(scope) == "1.2.3.4"

    def test_missing_client_returns_empty(self: "TestExtractClientIpNoTrustedProxies") -> None:
        scope = {"type": "http", "client": None, "headers": []}
        assert _extract_client_ip(scope) == ""

    def test_empty_peer_returns_empty(self: "TestExtractClientIpNoTrustedProxies") -> None:
        scope = {"type": "http", "client": ("", 0), "headers": []}
        assert _extract_client_ip(scope) == ""

    def test_malformed_peer_returns_empty(self: "TestExtractClientIpNoTrustedProxies") -> None:
...
class TestExtractClientIpWithTrustedProxies:
    ...
    def patch_trusted_proxies(self: "TestExtractClientIpWithTrustedProxies") -> Generator[None, None, None]:
        trusted = [ipaddress.ip_network("10.0.0.0/8")]
        with patch("supervaizer.access.client_ip.TRUSTED_PROXIES", trusted):
            yield

    def test_peer_in_trusted_cidr_uses_xff(self: "TestExtractClientIpWithTrustedProxies") -> None:
        scope = _make_scope("10.0.0.1", xff="203.0.113.5, 10.0.0.1")
        assert _extract_client_ip(scope) == "203.0.113.5"

    def test_peer_not_in_trusted_cidr_ignores_xff(self: "TestExtractClientIpWithTrustedProxies") -> None:
        scope = _make_scope("1.2.3.4", xff="9.9.9.9")
        assert _extract_client_ip(scope) == "1.2.3.4"

    def test_trusted_peer_no_xff_returns_peer(self: "TestExtractClientIpWithTrustedProxies") -> None:
        scope = _make_scope("10.0.0.2")
        assert _extract_client_ip(scope) == "10.0.0.2"

    def test_trusted_peer_malformed_xff_falls_back_to_peer(self: "TestExtractClientIpWithTrustedProxies") -> None:
        scope = _make_scope("10.0.0.1", xff="not-an-ip, 10.0.0.1")
        assert _extract_client_ip(scope) == "10.0.0.1"

    def test_trusted_peer_empty_xff_returns_peer(self: "TestExtractClientIpWithTrustedProxies") -> None:
